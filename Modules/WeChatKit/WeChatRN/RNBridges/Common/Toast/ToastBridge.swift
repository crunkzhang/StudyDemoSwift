...
@objc(ToastBridge)
final class ToastBridge: NSObject {

    @objc static func requiresMainQueueSetup() -> Bool { true }

    @objc func show(_ payload: NSDictionary) {
        ...
        DispatchQueue.main.async {
            ...
            ToastBridgeHandler.shared.show(
                message: toastParams.message,
                duration: toastParams.resolvedDuration
            )
        }
    }
}
...
final class ToastBridgeHandler {
    ...
    func show(message: String, duration: TimeInterval) {
        guard let containerView = RNContext.shared.currentViewController?.view else {
            return
        }
